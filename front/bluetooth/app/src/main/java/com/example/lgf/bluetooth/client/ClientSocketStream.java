...
public class ClientSocketStream extends Thread {
    private final BluetoothSocket mmSocket;
    private final InputStream mmInStream;
    private final OutputStream mmOutStream;
    private final Handler msgHandler;

    public ClientSocketStream(BluetoothSocket socket, Handler handler) {
        mmSocket = socket;
    ...
    public void run() {
        ...
        while (true) {
            try {
                mmOutStream.write("kkk".getBytes(Constant.ENCODE));
                // 从InputStream读取数据
